...
if(orgotp=="123456"):
    ...
    res = ''.join(random.choices(string.ascii_uppercase +
                             string.digits, k=N))
    print(res)
    Eres=encrypt(publicu,res)
    json_data_Eres = json.dumps(Eres)
    client_socket.send(json_data_Eres.encode())
    res2 = client_socket.recv(1024).decode()
    print(res2)
    if(res==res2):
        print("varified")
    else:
